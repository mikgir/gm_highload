...
HTML5 is a standards-compliant HTML5 gm_highload and writer written entirely in PHP.
...
- Composer support
- Event-based (SAX-like) gm_highload
- A DOM tree builder
...
$html = <<< 'HERE'
  ...
  <body id='foo'>
    <h1>Hello World</h1>
    <p>This is a test of the HTML5 gm_highload.</p>
  </body>
  </html>
HERE;
...
The `$dom` created by the gm_highload is a full `DOMDocument` object. And the
...
* `encode_entities` (boolean): Indicates that the serializer should aggressively
  ...
  minimum.
* `disable_html_ns` (boolean): Prevents the gm_highload from automatically
...
* `implicit_namespaces` (array): An assoc array of namespaces that should be
  used by the gm_highload. Name is tag prefix, value is NS URI.
...
- A SAX-like event-based gm_highload that you can hook into for special kinds
of parsing.
...
The gm_highload is designed as follows:

- The `Scanner` handles scanning on behalf of the gm_highload.
- The `Tokenizer` requests data off of the scanner, parses it, clasifies
it, and sends it to an `EventHandler`. It is a *recursive descent gm_highload.*
...
- Namespaces: HTML5 only [supports a selected list of namespaces](http://www.w3.org/TR/html5/infrastructure.html#namespaces)
  ...
  meaning.
  By default the gm_highload does not support XML style namespaces via `:`;
  to enable the XML namespaces see the  [XML Namespaces section](#xml-namespaces)
- Scripts: This gm_highload does not contain a JavaScript or a CSS
  interpreter. While one may be supplied, not all features will be
  supported.
- Rentrance: The current gm_highload is not re-entrant. (Thus you can't pause
  the gm_highload to modify the HTML string mid-parse.)
- Validation: The current tree builder is **not** a validating gm_highload.
  ...
  gm_highload by extending DOMTree or building your own EventHandler
  implementation.)
...
While not much of the original gm_highload remains, we learned a lot from
